    ...
    for (var i = 0; i < document.forms[0].length; i++) {
        ...
        if (elemento.value.trim() == '') {
            if (elemento.id == 'cedulaID') {
             ...
             if (elemento.id == 'operadoraID') {
                 nombres = elemento.id.value;
                 elemento.style.border = "1px red solid"
                 document.getElementById("operadoramss").innerHTML = "El campo es obligatorio"
                 document.getElementById("operadoramss").style.color = "red"
             } 
             

            if (elemento.id == 'apellidosID') {
...
function validar(label, elemento) {
    ...
    if (cad !== "" && longitud === 10) {
        ...
        if (cad.charAt(longitud - 1) == total) {
            span.innerHTML = "Cedula Valida";
            span.style.display = "block"
            span.style.color = "green"
        } else {
            span.innerHTML = "Cedula Invalida";
            span.style.display = "block"
...
function ValidarTelefono(event, label, elemento) {
    ...
    if(selec.length==8){
        if (elemento.value.length >= 7) {
            validar(label, elemento);
            span.innerHTML = "Solo ingrese siete numeros"
            span.style.display = "block"
            span.style.color = "red"
            let cantidad = elemento.value;
            cantidad = cantidad.substring(0, cantidad.length - 1)
